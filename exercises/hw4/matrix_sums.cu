...
__global__ void row_sums(const float *A, float *sums, size_t ds){
  ...
  int idx = FIXME // 用内置变量创建典型的一维线程索引
  if (idx < ds){
    float sum = 0.0f;
    for (size_t i = 0; i < ds; i++)
      sum += A[FIXME]         // 编写一个 for 循环，让线程遍历一整行、持续累加，并将结果写入 sums
    sums[idx] = sum;
}}
...
__global__ void column_sums(const float *A, float *sums, size_t ds){
  ...
  if (idx < ds){
    float sum = 0.0f;
    for (size_t i = 0; i < ds; i++)
      sum += A[FIXME]         // 编写一个 for 循环，让线程沿列向下遍历、持续累加，并将结果写入 sums
    sums[idx] = sum;
}}
